...
const ManageMyPost = () => {
  ...
  useEffect(() => {
    axios.get(`https://volunteer-management-server-beta.vercel.app/need-volunteer-my-post?email=${user.email}`, {
      withCredentials: true
    })
      .then(res => setPosts(res.data))

    axios.get(`https://volunteer-management-server-beta.vercel.app/be-a-volunteer?email=${user.email}`, {
      withCredentials: true
    })
  ...
  const handleRequestDelete = id => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://volunteer-management-server-beta.vercel.app/be-a-volunteer/${id}`)
          .then(res => {
  ...
  const handleNeedVolPostsDelete = id => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://volunteer-management-server-beta.vercel.app/need-volunteer-posts/${id}`)
          .then(res => {
            if (res.data.deletedCount) {
              Swal.fire({
                title: "Deleted!",
